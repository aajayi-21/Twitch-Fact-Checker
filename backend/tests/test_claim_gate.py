"""ClaimGate throttling, batching, context-tail handling, and error paths.

The buffering/throttle behavior lives on the provider-neutral base class and
is exercised here through the OpenRouter transport; that transport's own
request shapes and fallbacks have their suite in ``test_llm_openrouter.py``.
"""

import time

import pytest

from app.claim_gate import ClaimGate, GateError
from app.llm_openrouter import OpenRouterClaimGate
from app.models import GateClaim, TranscriptSegment
from tests.conftest import (
    TEST_GATE_MODEL,
    FakeLLMClient,
    make_chat_completion,
    make_gate_response,
    make_openrouter_timeout_error,
)

REAL_INPUT_MARKER = "CONTEXT (reference resolution only):"


def make_segment(text: str) -> TranscriptSegment:
    return TranscriptSegment(
        text=text, start=0.0, end=1.0, avg_logprob=-0.3, no_speech_prob=0.05
    )


def make_gate(
    client: FakeLLMClient,
    gate_interval_s: float = 100.0,
    gate_timeout_s: float = 5.0,
) -> OpenRouterClaimGate:
    return OpenRouterClaimGate(
        client,  # type: ignore[arg-type] — duck-typed fake
        model=TEST_GATE_MODEL,
        gate_interval_s=gate_interval_s,
        gate_timeout_s=gate_timeout_s,
        reasoning_effort="none",
    )


def real_input_sections(prompt: str) -> tuple[str, str]:
    """Extract the (context, new_transcript) of the final real-input block.

    The marker also appears inside the prompt's few-shot examples, so only
    the LAST occurrence is the real input.
    """
    tail = prompt.rsplit(REAL_INPUT_MARKER, 1)[1]
    context_part, new_part = tail.split("NEW TRANSCRIPT:", 1)
    return context_part.strip(), new_part.strip()


EIGHT_WORDS = "the eiffel tower is four hundred fifty meters"


class TestShouldRun:
    def test_false_without_enough_new_words(self, fake_llm_client) -> None:
        gate = make_gate(fake_llm_client)
        gate.add_transcript(make_segment("only three words"))
        assert gate.should_run(time.monotonic()) is False

    def test_true_once_min_words_accumulate(self, fake_llm_client) -> None:
        gate = make_gate(fake_llm_client)
        gate.add_transcript(make_segment(EIGHT_WORDS))
        assert gate.should_run(time.monotonic()) is True

    def test_words_accumulate_across_segments(self, fake_llm_client) -> None:
        gate = make_gate(fake_llm_client)
        gate.add_transcript(make_segment("four words right here"))
        gate.add_transcript(make_segment("and four more words"))
        assert gate.should_run(time.monotonic()) is True

    async def test_interval_throttles_after_a_run(self, fake_llm_client) -> None:
        gate = make_gate(fake_llm_client, gate_interval_s=100.0)
        gate.add_transcript(make_segment(EIGHT_WORDS))
        fake_llm_client.gate_results.append(make_gate_response([]))
        await gate.run()
        gate.add_transcript(make_segment(EIGHT_WORDS))
        now = time.monotonic()
        assert gate.should_run(now) is False  # interval not elapsed
        assert gate.should_run(now + 200.0) is True

    def test_whitespace_segments_are_ignored(self, fake_llm_client) -> None:
        gate = make_gate(fake_llm_client)
        gate.add_transcript(make_segment("   "))
        assert gate.should_run(time.monotonic()) is False


class TestRun:
    async def test_empty_buffer_returns_without_llm_call(self, fake_llm_client) -> None:
        gate = make_gate(fake_llm_client)
        assert await gate.run() == []
        assert fake_llm_client.gate_calls == []

    async def test_run_batches_all_pending_text(self, fake_llm_client) -> None:
        gate = make_gate(fake_llm_client)
        gate.add_transcript(make_segment("first chunk of words"))
        gate.add_transcript(make_segment("second chunk of words"))
        fake_llm_client.gate_results.append(
            make_gate_response([("The Earth is round.", 0.9)])
        )
        claims = await gate.run()
        assert [(c.claim_text, c.check_worthiness) for c in claims] == [
            ("The Earth is round.", 0.9)
        ]
        _context, new_text = real_input_sections(
            fake_llm_client.gate_calls[0]["messages"][0]["content"]
        )
        assert new_text == "first chunk of words second chunk of words"

    async def test_first_run_has_empty_context(self, fake_llm_client) -> None:
        gate = make_gate(fake_llm_client)
        gate.add_transcript(make_segment(EIGHT_WORDS))
        fake_llm_client.gate_results.append(make_gate_response([]))
        await gate.run()
        context, _ = real_input_sections(
            fake_llm_client.gate_calls[0]["messages"][0]["content"]
        )
        assert context == "(none)"

    async def test_context_tail_carries_previous_batch(self, fake_llm_client) -> None:
        gate = make_gate(fake_llm_client)
        gate.add_transcript(make_segment("the great wall of china came up"))
        fake_llm_client.gate_results.extend(
            [make_gate_response([]), make_gate_response([])]
        )
        await gate.run()
        gate.add_transcript(make_segment("and someone said you can see it from space"))
        await gate.run()
        context, new_text = real_input_sections(
            fake_llm_client.gate_calls[1]["messages"][0]["content"]
        )
        assert context == "the great wall of china came up"
        assert new_text == "and someone said you can see it from space"

    async def test_context_tail_capped_at_forty_words(self, fake_llm_client) -> None:
        gate = make_gate(fake_llm_client)
        words = [f"w{i}" for i in range(50)]
        gate.add_transcript(make_segment(" ".join(words)))
        fake_llm_client.gate_results.extend(
            [make_gate_response([]), make_gate_response([])]
        )
        await gate.run()
        gate.add_transcript(make_segment("fresh words for the second pass"))
        await gate.run()
        context, _ = real_input_sections(
            fake_llm_client.gate_calls[1]["messages"][0]["content"]
        )
        assert context.split() == words[-ClaimGate.CONTEXT_TAIL_WORDS :]

    async def test_failed_batch_is_dropped_not_retried(self, fake_llm_client) -> None:
        gate = make_gate(fake_llm_client)
        gate.add_transcript(make_segment(EIGHT_WORDS))
        fake_llm_client.gate_results.append(RuntimeError("api exploded"))
        with pytest.raises(GateError, match="gate call failed"):
            await gate.run()
        # Buffer was drained BEFORE the call: a second run has nothing to send.
        assert await gate.run() == []
        assert len(fake_llm_client.gate_calls) == 1

    async def test_timeout_raises_gate_error(self, fake_llm_client) -> None:
        # The OpenRouter transport bounds the call with the SDK's own
        # timeout (``with_options``), which surfaces as APITimeoutError.
        gate = make_gate(fake_llm_client, gate_timeout_s=0.05)
        gate.add_transcript(make_segment(EIGHT_WORDS))
        fake_llm_client.gate_results.append(make_openrouter_timeout_error())
        with pytest.raises(GateError, match="timed out"):
            await gate.run()


class TestExtractClaims:
    async def test_parses_strict_json_claims(self, fake_llm_client) -> None:
        gate = make_gate(fake_llm_client)
        fake_llm_client.gate_results.append(
            make_gate_response([("Messi has won eight Ballon d'Or awards.", 0.8)])
        )
        claims = await gate.extract_claims("", "some transcript text")
        assert claims[0].claim_text == "Messi has won eight Ballon d'Or awards."

    async def test_topic_is_parsed(self, fake_llm_client) -> None:
        gate = make_gate(fake_llm_client)
        raw = (
            '{"claims": [{"claim_text": "The sun is a star.", '
            '"check_worthiness": 0.6, "topic": "science_tech"}]}'
        )
        fake_llm_client.gate_results.append(make_chat_completion(raw))
        claims = await gate.extract_claims("", "some transcript text")
        assert claims[0].check_worthiness == 0.6
        assert claims[0].topic == "science_tech"

    async def test_missing_topic_defaults_to_other(self, fake_llm_client) -> None:
        gate = make_gate(fake_llm_client)
        raw = '{"claims": [{"claim_text": "The sun is a star.", "check_worthiness": 0.6}]}'
        fake_llm_client.gate_results.append(make_chat_completion(raw))
        claims = await gate.extract_claims("", "some transcript text")
        assert claims[0].topic == "other"

    async def test_unparseable_text_raises_gate_error(self, fake_llm_client) -> None:
        gate = make_gate(fake_llm_client)
        fake_llm_client.gate_results.append(make_chat_completion("not json at all"))
        with pytest.raises(GateError, match="no JSON object"):
            await gate.extract_claims("", "some transcript text")

    async def test_empty_response_raises_gate_error(self, fake_llm_client) -> None:
        gate = make_gate(fake_llm_client)
        fake_llm_client.gate_results.append(make_chat_completion(""))
        with pytest.raises(GateError, match="no text"):
            await gate.extract_claims("", "some transcript text")


class TestLastPassRecord:
    """ClaimGate.run() leaves a GatePass on last_pass for the pipeline."""

    class _ScriptedGate(ClaimGate):
        def __init__(self, result: object) -> None:
            super().__init__(gate_interval_s=0.0)
            self._result = result

        async def _extract(self, context: str, new_text: str) -> list:
            if isinstance(self._result, Exception):
                raise self._result
            return self._result

        async def judge_contradiction(self, current: str, prior: str):
            raise NotImplementedError

    @staticmethod
    def _segment(text: str) -> TranscriptSegment:
        return TranscriptSegment(
            text=text, start=0, end=1, avg_logprob=0.0, no_speech_prob=0.0
        )

    async def test_success_records_text_count_and_latency(self) -> None:
        claim = GateClaim(claim_text="X is Y.", check_worthiness=0.9, topic="other")
        gate = self._ScriptedGate([claim])
        gate.add_transcript(self._segment("x is y"))
        assert await gate.run() == [claim]
        record = gate.last_pass
        assert record.new_text == "x is y"
        assert record.claims_count == 1
        assert record.latency_ms is not None
        assert record.error is None and record.screen is None

    async def test_error_is_recorded_and_reraised(self) -> None:
        gate = self._ScriptedGate(GateError("boom"))
        gate.add_transcript(self._segment("x is y"))
        with pytest.raises(GateError):
            await gate.run()
        assert gate.last_pass.error == "boom"
        assert gate.last_pass.claims_count is None

    async def test_empty_run_and_debug_path_leave_no_record(self) -> None:
        gate = self._ScriptedGate([])
        gate.add_transcript(self._segment("x is y"))
        await gate.run()
        assert gate.last_pass is not None
        assert await gate.run() == []  # empty buffer
        assert gate.last_pass is None
        await gate.extract_claims("", "debug text")
        assert gate.last_pass is None
